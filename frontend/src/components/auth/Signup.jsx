...
const Signup = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      dispatch(setLoading(true))
         const res = await axios.post(`https://job-hunt-topaz-omega.vercel.app/api/user/register` , formData ,{
          headers:{
  ...
  return (
    <>
    <Navbar/>
    <div className="flex justify-center items-center min-h-screen my-4 bg-gray-50">
      <div className="w-full max-w-lg p-8 bg-white rounded-md shadow-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className='my-4'>
        <h1>Already have account ? <span className='text-blue-600 font-bold hover:text-blue-500' ><Link to="/login" >Login</Link></span></h1>
          </div>
        </form>
      </div>
    </div>
